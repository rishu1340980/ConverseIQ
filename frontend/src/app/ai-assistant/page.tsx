...
export default function AIAssistantPage() {
  ...
  const renderFormattedContent = (content: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (line.startsWith('### ')) {
        return (
          <h3 key={idx} className="font-bold text-sm text-[#1C251E] mt-2 mb-1">
            {line.replace('### ', '')}
          </h3>
        );
      }
      ...
      if (line.startsWith('## ')) {
        return (
          <h2 key={idx} className="font-bold text-base text-[#1C251E] mt-2.5 mb-1.5">
            {line.replace('## ', '')}
          </h2>
        );
      }
      ...
      if (line.startsWith('• ') || line.startsWith('- ') || line.startsWith('* ')) {
        ...
        return (
          <div key={idx} className="flex items-start space-x-2 my-1 pl-1">
            <span className="text-[#45644F] font-bold text-xs mt-0.5">•</span>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-5xl mx-auto space-y-4">
      ...
      {/* Top Banner Card */}
      <div className="bg-white rounded-2xl p-4 border border-[#E8E5DA] shadow-sm flex items-center justify-between flex-shrink-0">
        <div className="flex items-center space-x-3.5">
          <div className="w-10 h-10 rounded-xl bg-[#FEF3C7] text-[#D97706] flex items-center justify-center flex-shrink-0 shadow-xs">
        ...
        {messages.length > 1 && (
          <button
            ...
            className="flex items-center space-x-1.5 px-3 py-1.5 text-xs text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors border border-transparent hover:border-red-200"
          ...
          >
            ...
            >
              ...
              {m.sources && m.sources.length > 0 && (
                <div className="mt-3 pt-2.5 border-t border-[#E8E5DA]/70 flex flex-wrap items-center gap-1.5">
                  <span className="text-[10px] text-gray-400 font-medium flex items-center space-x-1 mr-1">
                  ...
                  {m.sources.map((src, i) => (
                    <span
                      key={i}
                      className="px-2 py-0.5 bg-[#F3EFE6] text-[#2F4E36] text-[10px] font-semibold rounded-md border border-[#E5E0D5]"
                    >
                      {src.title}
                    </span>
                  ))}
            ...
            {m.role === 'user' && (
              <div className="w-8 h-8 rounded-full bg-[#45644F] text-white flex items-center justify-center flex-shrink-0 mt-0.5 font-bold text-xs shadow-sm">
                {userInitial}
              </div>
            )}
          </div>
        ...
        {isLoading && (
          <div className="flex items-start space-x-3">
            ...
            <div className="bg-white rounded-2xl rounded-tl-sm p-4 border border-[#E8E5DA] shadow-sm flex items-center space-x-2 text-xs text-[#6B7280]">
              ...
              <span className="ml-1">Searching meeting context &amp; analyzing with Gemini...</span>
            </div>
          </div>
        )}
      ...
      <div className="space-y-3 flex-shrink-0 pt-2">
          ...
          <div className="flex flex-wrap gap-2">
            {suggestedQuestions.map((q) => (
              <button
                ...
                className="px-3.5 py-1.5 bg-white hover:bg-[#FAF9F5] border border-[#E8E5DA] rounded-full text-xs font-medium text-[#1C251E] shadow-xs transition-colors cursor-pointer"
              >
                {q}
              </button>
            ))}
        ...
        >
          ...
          <button
            ...
            className="w-9 h-9 rounded-full bg-[#45644F] hover:bg-[#385240] text-white flex items-center justify-center flex-shrink-0 transition-all shadow-sm disabled:opacity-40 cursor-pointer"
          >
