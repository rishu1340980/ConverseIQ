      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 bg-white rounded-2xl p-6 border border-[#E8E5DA] shadow-sm space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-[#1C251E]">
              {currentMonthName}
            </h2>
            <div className="flex items-center space-x-1 text-gray-400">
              <button className="p-1 hover:text-[#1C251E] rounded-md hover:bg-gray-100">
            ...
            {calendarDays.map((c, idx) => (
              <div key={idx} className="flex items-center justify-center h-8">
                {c.empty ? (
                  ...
                  <span className="hover:bg-gray-100 rounded-full w-7 h-7 flex items-center justify-center cursor-pointer">
                    {c.day}
                  </span>
                )}
              </div>
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-[#E8E5DA] shadow-sm space-y-4">
            <div className="flex items-center space-x-2 text-xs font-bold text-[#6B7280] tracking-wider uppercase">
              <CalendarIcon className="w-3.5 h-3.5 text-red-500" />
              <span>UPCOMING MEETINGS</span>
            </div>

            {upcomingMeetings.length > 0 ? (
              <div className="space-y-3 divide-y divide-[#E8E5DA]/60">
                {upcomingMeetings.map((m: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between pt-2 first:pt-0">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-[#1C251E]">{m.title}</p>
                        <p className="text-xs text-[#6B7280] mt-0.5">{m.date_info}</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="text-xs text-[#6B7280]">Remind</span>
                      <button
                        type="button"
                        onClick={() => toggleReminder(`meet-${idx}`)}
                        className={`w-9 h-5 flex items-center rounded-full p-0.5 transition-colors ${
                          reminders[`meet-${idx}`] ? 'bg-[#45644F]' : 'bg-gray-300'
                        }`}
                      >
                ...
                {facultySessions.map((s: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between pt-2 first:pt-0">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-[#1C251E]">{s.title}</p>
                        <p className="text-xs text-[#6B7280] mt-0.5">{s.date_info}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-white rounded-2xl p-5 border border-[#E8E5DA] shadow-sm space-y-4">
            ...
            {keyDeadlines.length > 0 ? (
                ...
                {keyDeadlines.map((d: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between pt-2 first:pt-0">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-[#1C251E]">{d.title}</p>
                        <p className="text-xs text-[#6B7280] mt-0.5">{d.date_info}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <p className="text-xs text-[#6B7280] py-3 text-center">No immediate deadlines pending.</p>
            )}
          </div>
